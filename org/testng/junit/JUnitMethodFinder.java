...
/**
 * This class locates all test and configuration methods according to JUnit. It
 * is used to change the strategy used by TestRunner to locate its test methods.
 * 
	...
	private ITestNGMethod[] privateFindTestMethods(INameFilter filter, Class cls) {
		...
		while (!(current == Object.class)) {
			...
			for (Method allMethod : allMethods) {
				ITestNGMethod m = new TestNGMethod(/*
													 * allMethods[i].
													 * getDeclaringClass(),
													 */allMethod,
						m_annotationFinder, null, null); /* @@@ */
				Method method = m.getMethod();
				String methodName = method.getName();
				if (filter.accept(method)
						&& !acceptedMethodNames.contains(methodName)) {
...
//			System.err
//					.println("Couldn't find a constructor with a String parameter on your JUnit test class.");
//			ex.printStackTrace();
