...
public class DependencyMap {
	// 维护了方法全名和ITestNGMethod列表的关系。构造器负责初始化
	private ListMultiMap<String, ITestNGMethod> m_dependencies = Maps
			.newListMultiMap();
	
	// 构造器负责初始化。由于小例子不涉及group所以集合为空
	private ListMultiMap<String, ITestNGMethod> m_groups = Maps
			.newListMultiMap();
	...
	public ITestNGMethod getMethodDependingOn(String methodName,
		...
		for (ITestNGMethod m : l) {
			// If they are in the same class hierarchy, they must belong to the
			// same instance,
			// otherwise, it's a method depending on a method in a different
			// class so we
