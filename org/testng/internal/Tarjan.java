 ...
 * 有向图的Tarjan算法。
 * 
 * Implementation of the Tarjan algorithm to find and display a cycle in a
 * graph.
 * 
 * @author cbeust
 */
public class Tarjan<T> {
	...
	private void run(Graph<T> graph, T v) {
		...
		for (T vprime : graph.getPredecessors(v)) {
			if (!m_indices.containsKey(vprime)) {
				...
				m_lowlinks.put(v,
						Math.min(m_lowlinks.get(v), m_indices.get(vprime)));
			}
		}
	...
	public static void main(String[] args) {
		...
		String[] edges = new String[] { "a", "b", "b", "a", "c", "d", "d", "a",
				"a", "c", };
