...
/**
 * 当前class的责任是执行测试方法已经配置方法。
 * 
 * This class is responsible for invoking methods: - test methods -
 * configuration methods - possibly in a separate thread and then for notifying
...
public class Invoker implements IInvoker {
	...
	private Map<Class<?>, Set<Object>> m_classInvocationResults = Maps
			.newHashtable();

	/** Test methods whose configuration methods have failed. */
	private Map<ITestNGMethod, Set<Object>> m_methodInvocationResults = Maps
			.newHashtable();
	...
	public Invoker(IConfiguration configuration, ITestContext testContext,
			ITestResultNotifier notifier, SuiteRunState state,
		...
		m_continueOnFailedConfiguration = XmlSuite.CONTINUE.equals(testContext
				.getSuite().getXmlSuite().getConfigFailurePolicy());
	}
	 ...
	 * in parameter..
	 * <p/>
	 * TODO: Calculate ahead of time which methods should be invoked for each
	 * class. Might speed things up for users who invoke the same test class
	 * with different parameters in the same suite run.
	 * 
	 * If instance is non-null, the configuration will be run on it. If it is
	 * null, the configuration methods will be run on all the instances
	 * retrieved from the ITestClass.
	 */
	@Override
	public void invokeConfigurations(IClass testClass,
			ITestNGMethod[] allMethods, XmlSuite suite,
			Map<String, String> params, Object[] parameterValues,
			Object instance) {
		invokeConfigurations(testClass, null, allMethods, suite, params,
				parameterValues, instance, null);
	} 
	
	private void invokeConfigurations(IClass testClass,
			ITestNGMethod currentTestMethod, ITestNGMethod[] allMethods,
		...
		ITestNGMethod[] methods = filterMethods(testClass, allMethods,
				SAME_CLASS);
		...
		for (ITestNGMethod tm : methods) {
			...
			ITestResult testResult = new TestResult(testClass, instance, tm,
					null, System.currentTimeMillis(),
					System.currentTimeMillis(), m_testContext);
			...
			try {
				...
				// - the Configuration method belongs to the same class or a
				// parent
				if (MethodHelper.isEnabled(objectClass, m_annotationFinder)) {
					...
					if (MethodHelper.isEnabled(configurationAnnotation)) {
						...
						if (!confInvocationPassed(tm, currentTestMethod,
								testClass, instance) && !alwaysRun) {
							handleConfigurationSkip(tm, testResult,
									configurationAnnotation, currentTestMethod,
									instance, suite);
							continue;
						}
						...
						Object[] parameters = Parameters
								.createConfigurationParameters(tm.getMethod(),
										params, parameterValues,
										currentTestMethod, m_annotationFinder,
										suite, m_testContext, testMethodResult);
						testResult.setParameters(parameters);

						Object[] newInstances = (null != instance) ? new Object[] { instance }
								: instances;

						runConfigurationListeners(testResult, true /* before */);

						invokeConfigurationMethod(newInstances, tm, parameters,
								isClassConfiguration, isSuiteConfiguration,
								testResult);

						// TODO: probably we should trigger the event for each
						// instance???
					...
					log(3, "Skipping " + Utils.detailedMethodName(tm, true)
							+ " because " + objectClass.getName()
							+ " is not enabled");
				}
			} catch (InvocationTargetException ex) {
				handleConfigurationFailure(ex, tm, testResult,
						configurationAnnotation, currentTestMethod, instance,
						suite);
			} catch (TestNGException ex) {
				// Don't wrap TestNGExceptions, it could be a missing parameter
				// on a
	...
	private void handleConfigurationSkip(ITestNGMethod tm,
			ITestResult testResult, IConfigurationAnnotation annotation,
			ITestNGMethod currentTestMethod, Object instance, XmlSuite suite) {
		recordConfigurationInvocationFailed(tm, testResult.getTestClass(),
				annotation, currentTestMethod, instance, suite);
	 ...
	 */
	private boolean isClassConfiguration(
	 ...
	 */
	private boolean isSuiteConfiguration(
			IConfigurationAnnotation configurationAnnotation) {
		if (null == configurationAnnotation) {
			return false;
		}
	...
	private boolean isAlwaysRun(IConfigurationAnnotation configurationAnnotation) {
		...
		if ((configurationAnnotation.getAfterSuite()
				...
				|| configurationAnnotation.getAfterTestClass() || configurationAnnotation
					.getAfterTestMethod())
	...
	private void handleConfigurationFailure(Throwable ite, ITestNGMethod tm,
			ITestResult testResult, IConfigurationAnnotation annotation,
			ITestNGMethod currentTestMethod, Object instance, XmlSuite suite) {
		...
		if (SkipException.class.isAssignableFrom(cause.getClass())) {
			...
			if (skipEx.isSkip()) {
				...
				handleConfigurationSkip(tm, testResult, annotation,
						currentTestMethod, instance, suite);
				return;
			}
		}
		Utils.log("", 3, "Failed to invoke configuration method "
				+ tm.getRealClass().getName() + "." + tm.getMethodName() + ":"
				+ cause.getMessage());
		handleException(cause, tm, testResult, 1);
		runConfigurationListeners(testResult, false /* after */);

		//
		// If in TestNG mode, need to take a look at the annotation to figure
		// out
	...
	private XmlClass[] findClassesInSameTest(Class<?> cls, XmlSuite suite) {
		...
		for (XmlTest test : suite.getTests()) {
			for (XmlClass testClass : test.getXmlClasses()) {
				if (testClass.getName().equals(className)) {
					...
					// Note: we need to iterate through the entire suite since
					// the same
		...
		XmlClass[] result = vResult.values().toArray(
				new XmlClass[vResult.size()]);
	 ...
	 * determine later if @Test should be skipped.
	 */
	private void recordConfigurationInvocationFailed(ITestNGMethod tm,
			IClass testClass, IConfigurationAnnotation annotation,
			ITestNGMethod currentTestMethod, Object instance, XmlSuite suite) {
		// If beforeTestClass or afterTestClass failed, mark either the config
		// method's
		// entire class as failed, or the class under tests as failed, depending
		// on
		// the configuration failure policy
		if (annotation.getBeforeTestClass() || annotation.getAfterTestClass()) {
			// tm is the configuration method, and currentTestMethod is null for
			// BeforeClass
		...
		// If before/afterTestMethod failed, mark either the config method's
		// entire
		// class as failed, or just the current test method as failed, depending
		// on
		// the configuration failure policy
		else if (annotation.getBeforeTestMethod()
	...
	private boolean confInvocationPassed(ITestNGMethod method,
			ITestNGMethod currentTestMethod, IClass testClass, Object instance) {
		boolean result = true;

		// If continuing on config failure, check invocation results for the
		// class
		// under test, otherwise use the method's declaring class
		Class<?> cls = m_continueOnFailedConfiguration ? testClass
				.getRealClass() : method.getMethod().getDeclaringClass();
		...
		if (m_suiteState.isFailed()) {
			...
			if (classConfigurationFailed(cls)) {
				if (!m_continueOnFailedConfiguration) {
					...
					result = !m_classInvocationResults.get(cls).contains(
							instance);
				}
			}
			// if method is BeforeClass, currentTestMethod will be null
				...
				result = !m_methodInvocationResults.get(currentTestMethod)
						.contains(
								getMethodInvocationToken(currentTestMethod,
										instance));
			} else if (!m_continueOnFailedConfiguration) {
	...
	// Creates a token for tracking a unique invocation of a method on an
	// instance.
	// Is used when configFailurePolicy=continue.
	private Object getMethodInvocationToken(ITestNGMethod method,
			Object instance) {
	 ...
	 * can handle calls to {@code IInvokedMethodListener} better.
	 * 
	 * @param instances
	 *            the instances to invoke the configuration method on
	 * @param tm
	 *            the configuration method
	 * @param params
	 *            the parameters needed for method invocation
	 * @param isClass
	 *            flag if the configuration method is a class level method //
	 *            FIXME: this looks like a missusage
	 * @param testResult
	 * @throws InvocationTargetException
	 * @throws IllegalAccessException
	 */
	private void invokeConfigurationMethod(Object[] instances,
			ITestNGMethod tm, Object[] params, boolean isClass,
			boolean isSuite, ITestResult testResult)
			throws InvocationTargetException, IllegalAccessException {
		// Mark this method with the current thread id
		tm.setId(ThreadUtil.currentThreadInfo());

		// Only a @BeforeMethod/@AfterMethod needs to be run before each
		// instance, all the other
		...
		for (Object targetInstance : actualInstances) {
			InvokedMethod invokedMethod = new InvokedMethod(targetInstance, tm,
					params, false, /* isTest */
					isClass, /* ??? */
					System.currentTimeMillis(), testResult);
			...
			try {
				...
				//
				IConfigurable configurableInstance = IConfigurable.class
						.isAssignableFrom(tm.getMethod().getDeclaringClass()) ? (IConfigurable) targetInstance
						: m_configuration.getConfigurable();
				if (configurableInstance != null) {
					//
					// If this method is a IConfigurable, invoke its run()
					// method
					//
					MethodInvocationHelper.invokeConfigurable(targetInstance,
							params, configurableInstance, method, testResult);
				} else {
					//
					...
					if (MethodHelper.calculateTimeOut(tm) <= 0) {
						...
						MethodInvocationHelper.invokeWithTimeout(tm,
								targetInstance, params, testResult);
						if (!testResult.isSuccess()) {
							// A time out happened
							throwConfigurationFailure(testResult,
				...
				Reporter.setCurrentTestResult(testResult);
				runInvokedMethodListeners(AFTER_INVOCATION, invokedMethod,
						testResult);
	...
	private void runInvokedMethodListeners(
			...
			IInvokedMethod invokedMethod, ITestResult testResult) {
		if (noListenersPresent()) {
			return;
		}

		InvokedMethodListenerInvoker invoker = new InvokedMethodListenerInvoker(
				listenerMethod, testResult, m_testContext);
	...
	private boolean noListenersPresent() {
		return (m_invokedMethodListeners == null)
				|| (m_invokedMethodListeners.size() == 0);
	}

	// pass both paramValues and paramIndex to be thread safe in case
	// parallel=true + dataprovider.
	private ITestResult invokeMethod(Object[] instances, int instanceIndex,
			final ITestNGMethod tm, Object[] parameterValues,
			int parametersIndex, XmlSuite suite, Map<String, String> params,
			ITestClass testClass, ITestNGMethod[] beforeMethods,
			ITestNGMethod[] afterMethods, ConfigurationGroupMethods groupMethods) {
		...
		invokeBeforeGroupsConfigurations(testClass, tm, groupMethods, suite,
				params, instance);

		//
		// Invoke beforeMethods only if
		// - firstTimeOnly is not set
		// - firstTimeOnly is set, and we are reaching at the first
		// invocationCount
		//
		invokeConfigurations(
				...
				suite, params, parameterValues, instance, testResult);
		...
		try {
			testResult.init(testClass, instance, tm, null,
					System.currentTimeMillis(), 0, m_testContext);
			testResult.setParameters(parameterValues);
			testResult.setHost(m_testContext.getHost());
			testResult.setStatus(ITestResult.STARTED);

			invokedMethod = new InvokedMethod(instance, tm, parameterValues,
					true /* isTest */, false /* isConfiguration */,
					System.currentTimeMillis(), testResult);
			...
			if (confInvocationPassed(tm, tm, testClass, instance)) {
				...
				if (MethodHelper.calculateTimeOut(tm) <= 0) {
					...
					// If this method is a IHookable, invoke its run() method
					//
					IHookable hookableInstance = IHookable.class
							.isAssignableFrom(thisMethod.getDeclaringClass()) ? (IHookable) instance
							: m_configuration.getHookable();
					if (hookableInstance != null) {
						MethodInvocationHelper.invokeHookable(instance,
								parameterValues, hookableInstance, thisMethod,
								testResult);
					}
					//
					...
					//
						...
						MethodInvocationHelper.invokeMethod(thisMethod,
					...
					//
					Reporter.setCurrentTestResult(testResult);
					MethodInvocationHelper.invokeWithTimeout(tm, instance,
			...
			testResult.setStatus(ITestResult.FAILURE);
		} catch (ThreadExecutionException tee) { // wrapper for
			...
			ExpectedExceptionsHolder expectedExceptionClasses = MethodHelper
					.findExpectedExceptions(m_annotationFinder, tm.getMethod());
			List<ITestResult> results = Lists.newArrayList();
			results.add(testResult);
			handleInvocationResults(tm, results, null, 0,
					expectedExceptionClasses, false, false /* collect results */);

			// If this method has a data provider and just failed, memorize the
			// number
			// at which it failed.
			// Note: we're not exactly testing that this method has a data
			// provider, just
			// that it has parameters, so might have to revisit this if bugs get
			// reported
			// for the case where this method has parameters that don't come
			// from a data
			// provider
			...
			// - lastTimeOnly is set, and we are reaching the last
			// invocationCount
			//
			invokeConfigurations(
					testClass,
			...
			// Reset the test result last. If we do this too early,
			// Reporter.log()
	...
	private void collectResults(ITestNGMethod testMethod,
			List<ITestResult> results, TestResult testResult) {
		for (int i = 0; i < results.size(); i++) {
			...
			if (ITestResult.SUCCESS == status) {
				...
				m_notifier.addFailedButWithinSuccessPercentageTest(testMethod,
						testResult);
			} else {
	 ...
	 * at this point because they are either firstTimeOnly or lastTimeOnly and
	 * we haven't reached the current invocationCount yet
	 */
		...
		for (ITestNGMethod m : methods) {
			...
			if (isBefore) {
				if (!cm.isFirstTimeOnly()
						|| (cm.isFirstTimeOnly() && tm
								.getCurrentInvocationCount() == 0)) {
					result.add(m);
				}
			} else {
				...
				// If we have invocationCount > 1, set the boolean if we are
				// about to
	...
	/**
	 * {@link #invokeTestMethods()} eventually converge here to invoke a single @Test
	 * method.
	 * <p/>
	 * This method is responsible for actually invoking the method. It decides
	 * if the invocation must be done:
	 * <ul>
	 ...
	 * 
	 * <p/>
	 * This method is also reponsible for invoking @BeforeGroup, @BeforeMethod,
	 * @AfterMethod, @AfterGroup if it is the case for the passed in @Test
	 * method.
	 */
	protected List<ITestResult> invokeTestMethod(Object[] instances,
		...
		for (int i = 0; i < instances.length; i++) {
			results.add(invokeMethod(instances, i, tm, parameterValues,
					parametersIndex, suite, params, testClass, beforeMethods,
					afterMethods, groupMethods));
		}
	...
	/**
	 * Filter all the beforeGroups methods and invoke only those that apply to
	 * the current test method
	 */
	private void invokeBeforeGroupsConfigurations(ITestClass testClass,
			ITestNGMethod currentTestMethod,
			ConfigurationGroupMethods groupMethods, XmlSuite suite,
		...
		synchronized (groupMethods) {
			...
			Map<String, List<ITestNGMethod>> beforeGroupMap = groupMethods
					.getBeforeGroupsMap();
			...
			ITestNGMethod[] beforeMethodsArray = filteredMethods
					.toArray(new ITestNGMethod[filteredMethods.size()]);
			//
			// Invoke the right groups methods
			//
			if (beforeMethodsArray.length > 0) {
				// don't pass the IClass or the instance as the method may be
				// external
	...
	private void invokeAfterGroupsConfigurations(ITestClass testClass,
		...
		synchronized (groupMethods) {
			...
			// Now filteredGroups contains all the groups for which we need to
			// run the afterGroups
			// method. Find all the methods that correspond to these groups and
			// invoke them.
			Map<String, List<ITestNGMethod>> map = groupMethods
					.getAfterGroupsMap();
			for (String g : filteredGroups.values()) {
				List<ITestNGMethod> methods = map.get(g);
				// Note: should put them in a map if we want to make sure the
				// same afterGroups
			...
			ITestNGMethod[] afterMethodsArray = afterMethods.keySet().toArray(
					new ITestNGMethod[afterMethods.size()]);
			// don't pass the IClass or the instance as the method may be
			// external
			// the invocation must be similar to @BeforeTest/@BeforeSuite
	...
	private Object[] getParametersFromIndex(
			Iterator<Object[]> parametersValues, int index) {
	...
	int retryFailed(Object[] instances, int instanceIndex,
			final ITestNGMethod tm, XmlSuite suite, ITestClass testClass,
			ITestNGMethod[] beforeMethods, ITestNGMethod[] afterMethods,
			ConfigurationGroupMethods groupMethods, List<ITestResult> result,
			int failureCount, ExpectedExceptionsHolder expectedExceptionHolder,
			ITestContext testContext, Map<String, String> parameters,
			int parametersIndex) {
		...
		do {
			 ...
			 * need one specific set. Should optimize it by only recreating the
			 * set needed.
			 */
			ParameterBag bag = createParameters(tm, parameters, allParameters,
					null, suite, testContext, null /* fedInstance */, null /* testResult */);
			Object[] parameterValues = getParametersFromIndex(
					bag.parameterHolder.parameters, parametersIndex);

			result.add(invokeMethod(instances, instanceIndex, tm,
					parameterValues, parametersIndex, suite, allParameters,
					testClass, beforeMethods, afterMethods, groupMethods));
			failureCount = handleInvocationResults(tm, result, failedInstances,
					failureCount, expectedExceptionHolder, true, true /*
																	 * collect
																	 * results
																	 */);
	...
	private ParameterBag createParameters(ITestNGMethod testMethod,
			...
			Map<String, String> allParameterNames, Object[] parameterValues,
			XmlSuite suite, ITestContext testContext, Object fedInstance,
		...
		ParameterBag bag = handleParameters(testMethod, instance,
				allParameterNames, parameters, parameterValues, suite,
				testContext, fedInstance, testResult);
	 ...
	 * parameter might be invoked several times if the test class it belongs to
	 * has more than one instance (i.e., if an @Factory method has been declared
	 * somewhere that returns several instances of this TestClass). If no @Factory
	 * method was specified, testMethod will only be invoked once.
	 * <p/>
	 ...
	 * SingleTestMethodWorker that directly invokes
	...
	public List<ITestResult> invokeTestMethods(ITestNGMethod testMethod,
		...
		if (!MethodHelper.isEnabled(testMethod.getMethod(), m_annotationFinder)) {
			/*
			 * return if the method is not enabled. No need to do any more
			 * calculations
			 */
			return result;
		}
		...
		// For invocationCount > 1 and threadPoolSize > 1 the method will be
		// invoked on a thread pool
		...
		ExpectedExceptionsHolder expectedExceptionHolder = MethodHelper
				.findExpectedExceptions(m_annotationFinder,
		...
		while (invocationCount-- > 0) {
			...
			if (!okToProceed) {
				...
				ITestResult testResult = new TestResult(testClass,
						null /* instance */, testMethod, null /* cause */, start,
						System.currentTimeMillis(), m_testContext);
				String missingGroup = testMethod.getMissingGroup();
				if (missingGroup != null) {
					testResult.setThrowable(new Throwable("Method "
							+ testMethod + " depends on nonexistent group \""
							+ missingGroup + "\""));
				}
			...
			//
			// If threadPoolSize specified, run this method in its own pool
			// thread.
			//
			Map<String, String> parameters = testMethod
					.findMethodParameters(testContext.getCurrentXmlTest());
			if (testMethod.getThreadPoolSize() > 1
					&& testMethod.getInvocationCount() > 1) {
				return invokePooledTestMethods(testMethod, allTestMethods,
						suite, parameters, groupMethods, testContext);
			}
			//
			// No threads, regular invocation
			//
			else {
				ITestNGMethod[] beforeMethods = filterMethods(testClass,
						testClass.getBeforeTestMethods(), CAN_RUN_FROM_CLASS);
				ITestNGMethod[] afterMethods = filterMethods(testClass,
						testClass.getAfterTestMethods(), CAN_RUN_FROM_CLASS);

				Map<String, String> allParameterNames = Maps.newHashMap();
				ParameterBag bag = createParameters(testMethod, parameters,
						allParameterNames, null, suite, testContext,
						instances[0], null);

				if (bag.hasErrors()) {
					failureCount = handleInvocationResults(testMethod,
							bag.errorResults, null, failureCount,
							expectedExceptionHolder, true, true /*
																 * collect
																 * results
																 */);
					ITestResult tr = registerSkippedTestResult(testMethod,
							instances[0], start, bag.errorResults.get(0)
									.getThrowable());
					result.add(tr);
					continue;
				}
				...
				try {
					List<TestMethodWithDataProviderMethodWorker> workers = Lists
							.newArrayList();

					if (bag.parameterHolder.origin == ParameterOrigin.ORIGIN_DATA_PROVIDER
							&& bag.parameterHolder.dataProviderHolder.annotation
									.isParallel()) {
						while (allParameterValues.hasNext()) {
							Object[] parameterValues = injectParameters(
									allParameterValues.next(),
									testMethod.getMethod(), testContext, null /*
																			 * test
																			 * result
																			 */);
							TestMethodWithDataProviderMethodWorker w = new TestMethodWithDataProviderMethodWorker(
									this, testMethod, parametersIndex,
									parameterValues, instances, suite,
									parameters, testClass, beforeMethods,
									afterMethods, groupMethods,
									expectedExceptionHolder, testContext,
						...
						PoolService<List<ITestResult>> ps = new PoolService<List<ITestResult>>(
								suite.getDataProviderThreadCount());
						List<List<ITestResult>> r = ps
								.submitTasksAndWait(workers);
						...
						while (allParameterValues.hasNext()) {
							...
							try {
								tmpResults.addAll(invokeTestMethod(instances,
										testMethod, parameterValues,
										parametersIndex, suite, parameters,
										testClass, beforeMethods, afterMethods,
										groupMethods));
							} finally {
								List<Object> failedInstances = Lists
										.newArrayList();

								failureCount = handleInvocationResults(
										testMethod, tmpResults,
										failedInstances, failureCount,
										expectedExceptionHolder, true, false /*
																			 * don't
																			 * collect
																			 * results
																			 */);
								if (failedInstances.isEmpty()) {
									...
									for (int i = 0; i < failedInstances.size(); i++) {
										List<ITestResult> retryResults = Lists
												.newArrayList();

										failureCount = retryFailed(
												failedInstances.toArray(), i,
												testMethod, suite, testClass,
												beforeMethods, afterMethods,
												groupMethods, retryResults,
												failureCount,
								...
								//
								if (failureCount > 0
										&& (m_skipFailedInvocationCounts || testMethod
												.skipFailedInvocations())) {
									while (invocationCount-- > 0) {
										result.add(registerSkippedTestResult(
												testMethod, instances[0],
												start, null));
									}
									break;
								}
					...
					ITestResult r = new TestResult(testMethod.getTestClass(),
							instances[0], testMethod, cause, start,
							System.currentTimeMillis(), m_testContext);
	...
	private ITestResult registerSkippedTestResult(ITestNGMethod testMethod,
			Object instance, long start, Throwable throwable) {
		ITestResult result = new TestResult(testMethod.getTestClass(),
				instance, testMethod, throwable, start,
				System.currentTimeMillis(), m_testContext);
	 ...
	 * that are injected based on parameter type. The method also checks for
	 * {@code NoInjection} annotation
	 * 
	 * @param parameterValues
	 *            parameter values from a data provider
	 * @param method
	...
	private Object[] injectParameters(Object[] parameterValues, Method method,
		...
		if (numValues > numParams && !method.isVarArgs()) {
			throw new TestNGException("The data provider is trying to pass "
					+ numValues + " parameters but the method "
					+ method.getDeclaringClass().getName() + "#"
					+ method.getName() + " takes " + numParams);
		}
		...
		for (Class<?> cls : method.getParameterTypes()) {
			...
			Object injected = Parameters.getInjectedParameter(cls, method,
					context, testResult);
			if (injected != null && !noInjection) {
				...
				try {
					...
					throw new TestNGException(
							"The data provider is trying to pass "
									+ numValues
									...
									+ " takes "
									+ numParams
									+ " and TestNG is unable in inject a suitable object",
							ex);
				}
			}
		}
	...
	private ParameterBag handleParameters(ITestNGMethod testMethod,
			Object instance, Map<String, String> allParameterNames,
			Map<String, String> parameters, Object[] parameterValues,
			XmlSuite suite, ITestContext testContext, Object fedInstance,
			ITestResult testResult) {
		try {
			return new ParameterBag(Parameters.handleParameters(
					testMethod,
					allParameterNames,
					instance,
					new Parameters.MethodParameters(parameters, testMethod
							.findMethodParameters(testContext
									.getCurrentXmlTest()), parameterValues,
							testMethod.getMethod(), testContext, testResult),
					suite, m_annotationFinder, fedInstance), null /* TestResult */);
		}
		// catch(TestNGException ex) {
		// throw ex;
		// }
		catch (Throwable cause) {
			return new ParameterBag(null /* ParameterHolder */, new TestResult(
					testMethod.getTestClass(), instance, testMethod, cause,
	...
	private List<ITestResult> invokePooledTestMethods(ITestNGMethod testMethod,
			ITestNGMethod[] allTestMethods, XmlSuite suite,
			Map<String, String> parameters,
			ConfigurationGroupMethods groupMethods, ITestContext testContext) {
		//
		// Create the workers
		//
		...
		for (int i = 0; i < testMethod.getInvocationCount(); i++) {
			...
			workers.add(new SingleTestMethodWorker(this, mi, suite, parameters,
					allTestMethods, testContext));
		}

		return runWorkers(testMethod, workers, testMethod.getThreadPoolSize(),
				groupMethods, suite, parameters);
	}
	...
	int handleInvocationResults(ITestNGMethod testMethod,
			List<ITestResult> result, List<Object> failedInstances,
			int failureCount,
			ExpectedExceptionsHolder expectedExceptionsHolder,
			boolean triggerListeners, boolean collectResults) {
		//
		...
		for (int i = 0; i < result.size(); i++) {
			...
			if (ite != null) {
				...
				// Invocation caused an exception, see if the method was
				// annotated with @ExpectedException
				if (isExpectedException(ite, expectedExceptionsHolder)) {
					if (messageRegExpMatches(
							expectedExceptionsHolder.messageRegExp, ite)) {
						testResult.setStatus(ITestResult.SUCCESS);
						status = ITestResult.SUCCESS;
					} else {
						testResult
								.setThrowable(new TestException(
										"The exception was thrown with the wrong message:"
												+ " expected \""
												+ expectedExceptionsHolder.messageRegExp
												+ "\"" + " but got \""
												+ ite.getMessage() + "\"", ite));
					...
					if (skipEx.isSkip()) {
						...
						handleException(ite, testMethod, testResult,
								failureCount++);
						status = ITestResult.FAILURE;
					}
			...
			// No exception thrown, make sure we weren't expecting one
					...
					&& expectedExceptionsHolder != null) {
				Class<?>[] classes = expectedExceptionsHolder.expectedClasses;
				if (classes != null && classes.length > 0) {
					testResult.setThrowable(new TestException("Method "
							+ testMethod
		...
		return removeResultsToRetryFromResult(resultsToRetry, result,
				failureCount);
	}
	...
	private int removeResultsToRetryFromResult(
			List<ITestResult> resultsToRetry, List<ITestResult> result,
			int failureCount) {
	 ...
	 * thread-confinement this method invokes the @BeforeGroups and @AfterGroups
	 * corresponding to the current @Test method.
	 */
	private List<ITestResult> runWorkers(ITestNGMethod testMethod,
			List<IWorker<ITestNGMethod>> workers, int threadPoolSize,
			ConfigurationGroupMethods groupMethods, XmlSuite suite,
			Map<String, String> parameters) {
		// Invoke @BeforeGroups on the original method (reduce thread
		// contention,
		// and also solve thread confinement)
		ITestClass testClass = testMethod.getTestClass();
		Object[] instances = testClass.getInstances(true);
		for (Object instance : instances) {
			invokeBeforeGroupsConfigurations(testClass, testMethod,
					groupMethods, suite, parameters, instance);
		}
		...
		for (Object instance : instances) {
			invokeAfterGroupsConfigurations(testClass, testMethod,
					groupMethods, suite, parameters, instance);
		}
	...
	private boolean checkDependencies(ITestNGMethod testMethod,
		...
		if (dependsOnGroups(testMethod)) {
			...
			for (String element : groupsDependedUpon) {
				ITestNGMethod[] methods = MethodGroupsHelper
						.findMethodsThatBelongToGroup(testMethod,
								m_testContext.getAllTestMethods(), element);
		...
		if (result && dependsOnMethods(testMethod)) {
			ITestNGMethod[] methods = MethodHelper.findDependedUponMethods(
					testMethod, allTestMethods);
	 ...
	 *         testMethod.
	 */
	private Set<ITestResult> keepSameInstances(ITestNGMethod method,
			Set<ITestResult> results) {
		Set<ITestResult> result = Sets.newHashSet();
		for (ITestResult r : results) {
			for (Object o : method.getInstances()) {
				// Keep this instance if 1) It's on a different class or 2) It's
				// on the same class
				// and on the same instance
				Object instance = r.getInstance() != null ? r.getInstance() : r
						.getMethod().getInstances()[0];
	...
	private boolean haveBeenRunSuccessfully(ITestNGMethod testMethod,
		...
		for (ITestNGMethod method : methods) {
			Set<ITestResult> results = keepSameInstances(testMethod,
					m_notifier.getPassedTests(method));
	...
	// private boolean containsInstance(Set<ITestResult> failedresults, Object[]
	// instances) {
	...
	/**
	 * An exception was thrown by the test, determine if this method should be
	 * marked as a failure or as failure_but_within_successPercentage
	 */
	private void handleException(Throwable throwable, ITestNGMethod testMethod,
			ITestResult testResult, int failureCount) {
	 ...
	 *            The list of expected exceptions for this test method
	 * @return true if the exception that was just thrown is part of the
	 *         expected exceptions
	 */
	private boolean isExpectedException(Throwable ite,
			ExpectedExceptionsHolder exceptionHolder) {
		if (exceptionHolder == null) {
			return false;
		}

		// TestException is the wrapper exception that TestNG will be throwing
	...
	static class CanRunFromClassPredicate implements
			Predicate<ITestNGMethod, IClass> {
		@Override
	...
	static class SameClassNamePredicate implements
			Predicate<ITestNGMethod, IClass> {
		@Override
	 ...
	 */
	private ITestNGMethod[] filterMethods(IClass testClass,
			ITestNGMethod[] methods, Predicate<ITestNGMethod, IClass> predicate) {
		...
		for (ITestNGMethod tm : methods) {
			if (predicate.isTrue(tm, testClass)) {
				...
				log(10, "Filtering out method " + tm + " for class "
						+ testClass);
			}
		}

		ITestNGMethod[] result = vResult.toArray(new ITestNGMethod[vResult
				.size()]);
	...
	private void runConfigurationListeners(ITestResult tr, boolean before) {
		if (before) {
			for (IConfigurationListener icl : m_notifier
	...
	/**
	 * This class holds a {@code ParameterHolder} and in case of an error, a
	 * non-null {@code TestResult} containing the cause
	 */
