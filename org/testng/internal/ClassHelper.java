...
public final class ClassHelper {
	...
	public static <T> T newInstance(Class<T> clazz) {
		try {
			...
			throw new TestNGException(
					"An exception occurred in static initialization of class "
	 ...
	 * the class is not found but returns null instead.
	 * 
	 * @param className
	...
	public static Class<?> forName(final String className) {
		...
		for (ClassLoader classLoader : allClassLoaders) {
			...
			try {
				...
				// With additional class loaders, it is legitimate to ignore
	...
	/**
	 * For the given class, returns the method annotated with &#64;Factory or
	 * null if none is found. This method does not search up the superclass
	 * hierarchy. If more than one method is @Factory annotated, a
	 * TestNGException is thrown.
	 * 
	 * @param cls
	 *            The class to search for the @Factory annotation.
	 * @param finder
	 *            The finder (JDK 1.4 or JDK 5.0+) use to search for the
	 *            annotation.
	 * 
	...
	public static ConstructorOrMethod findDeclaredFactoryMethod(Class<?> cls,
		...
		for (Method method : cls.getMethods()) {
			IFactoryAnnotation f = (IFactoryAnnotation) finder.findAnnotation(
					method, IFactoryAnnotation.class);

			if (null != f) {
				if (result != null) {
					throw new TestNGException(cls.getName()
							+ ":  only one @Factory method allowed");
				}
		...
		if (result == null) {
			for (Constructor constructor : cls.getDeclaredConstructors()) {
				IAnnotation f = finder.findAnnotation(constructor,
	...
	/**
	 * Extract all callable methods of a class and all its super (keeping in
	 * mind the Java access rules).
	 * 
	 * @param clazz
	 * @return
	 */
	...
	public static IJUnitTestRunner createTestRunner(TestRunner runner) {
		try {
			...
			IJUnitTestRunner tr = (IJUnitTestRunner) ClassHelper.forName(
					JUNIT_4_TESTRUNNER).newInstance();
			tr.setTestResultNotifier(runner);
			return tr;
		} catch (Throwable t) {
			Utils.log("ClassHelper", 2,
					"JUnit 4 was not found on the classpath");
			try {
				// fallback to JUnit 3
				Class.forName("junit.framework.Test");
				IJUnitTestRunner tr = (IJUnitTestRunner) ClassHelper.forName(
						JUNIT_TESTRUNNER).newInstance();
				tr.setTestResultNotifier(runner);
	...
	private static Set<Method> extractMethods(Class<?> childClass,
			Class<?> clazz, Set<Method> collected) {
		Set<Method> methods = Sets.newHashSet();
		...
		if ((null != childPackage) && (null != classPackage)) {
			isSamePackage = childPackage.getName().equals(
					classPackage.getName());
		}

		for (Method method : declaredMethods) {
			int methodModifiers = method.getModifiers();
			if ((Modifier.isPublic(methodModifiers) || Modifier
					.isProtected(methodModifiers))
	...
	public static IMethodSelector createSelector(
		...
		try {
			...
			throw new TestNGException("Couldn't find method selector : "
					+ selector.getClassName(), ex);
		}
	}
	...
	public static Object createInstance(Class<?> declaringClass,
		...
		if (objectFactory instanceof IObjectFactory) {
			...
			return createInstance2(declaringClass,
					(IObjectFactory2) objectFactory);
		} else {
			throw new AssertionError("Unknown object factory type:"
					+ objectFactory);
		}
	}

	private static Object createInstance2(Class<?> declaringClass,
			IObjectFactory2 objectFactory) {
	...
	public static Object createInstance1(Class<?> declaringClass,
			Map<Class, IClass> classes, XmlTest xmlTest,
			IAnnotationFinder finder, IObjectFactory objectFactory) {
		Object result = null;
		...
		try {
			...
			//
			Constructor<?> constructor = findAnnotatedConstructor(finder,
					declaringClass);
			if (null != constructor) {
				IParametersAnnotation annotation = (IParametersAnnotation) finder
						.findAnnotation(constructor,
								IParametersAnnotation.class);

				String[] parameterNames = annotation.getValue();
				Object[] parameters = Parameters.createInstantiationParameters(
						constructor, "@Parameters", finder, parameterNames,
						xmlTest.getAllParameters(), xmlTest.getSuite());
				result = objectFactory.newInstance(constructor, parameters);
			}

			//
			// No, just try to instantiate the parameterless constructor (or the
			// one
			// with a String)
			//
			else {

				// If this class is a (non-static) nested class, the constructor
				// contains a hidden
				...
				// Only add the extra parameter if the nested class is not
				// static
				if ((null != ec) && !isStatic) {
					parameterTypes = new Class[] { ec };

					// Create an instance of the enclosing class so we can
					// instantiate
					// the nested class (actually, we reuse the existing
					// instance).
					IClass enclosingIClass = classes.get(ec);
					Object[] enclosingInstances;
					if (null != enclosingIClass) {
						enclosingInstances = enclosingIClass
								.getInstances(false);
						if ((null == enclosingInstances)
								|| (enclosingInstances.length == 0)) {
							Object o = objectFactory.newInstance(ec
			...
			throw ex;
			// throw new TestNGException("Couldn't instantiate class:" +
			// declaringClass);
		} catch (NoSuchMethodException ex) {
		...
		if (result == null) {
			if (!Modifier.isPublic(declaringClass.getModifiers())) {
				...
				// } else {
				// Utils.log(ClassHelper.class.getName(), 2,
				// "Couldn't instantiate class " + declaringClass);
			}
		}
	...
	private static Constructor<?> findAnnotatedConstructor(
		...
		for (Constructor<?> result : constructors) {
			IParametersAnnotation annotation = (IParametersAnnotation) finder
					.findAnnotation(result, IParametersAnnotation.class);

			if (null != annotation) {
				String[] parameters = annotation.getValue();
				Class<?>[] parameterTypes = result.getParameterTypes();
				if (parameters.length != parameterTypes.length) {
					throw new TestNGException("Parameter count mismatch:  "
							+ result + "\naccepts " + parameterTypes.length
	...
	public static <T> T tryOtherConstructor(Class<T> declaringClass) {
		...
		try {
			...
			String error = "Could not create an instance of class "
					+ declaringClass
	 ...
	 * divided into segments. For example, "c:/java/classes/com/foo/A.class"
	 * would be divided into 6 segments {"C:" "java", "classes", "com", "foo",
	 * "A"}. The first segment actually making up the class name is [3]. This
	 * value is saved in m_lastGoodRootIndex so that when we parse the next file
	 * name, we will try 3 right away. If 3 fails we will take the long
	 * approach. This is just a optimization cache value.
	 */
	...
	public static Class<?> fileToClass(String file) {
		...
		if (!file.endsWith(".class") && !file.endsWith(".java")) {
			...
			if (null == result) {
				throw new TestNGException("Cannot load class from file: "
						+ file);
			}
		...
		// Split file name into segments. For example
		// "c:/java/classes/com/foo/A"
		// becomes {"c:", "java", "classes", "com", "foo", "A"}
		String[] segments = shortFileName.split("[/\\\\]", -1);

		//
		// Check if the last good root index works for this one. For example, if
		// the previous
		// name was "c:/java/classes/com/foo/A.class" then m_lastGoodRootIndex
		// is 3 and we
		// try to make a class name ignoring the first m_lastGoodRootIndex
		// segments (3). This
		// will succeed rapidly if the path is the same as the one from the
		// previous name.
		//
		...
		//
		// We haven't found a good root yet, start by resolving the class from
		// the end segment
		// and work our way up. For example, if we start with
		// "c:/java/classes/com/foo/A"
		// we'll start by resolving "A", then "foo.A", then "com.foo.A" until
		// something
		// resolves. When it does, we remember the path we are at as
		// "lastGoodRoodIndex".
		//
