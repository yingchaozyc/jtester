 ...
 * 很重要，包含了决定哪些方法要执行的信息。
 * 
 * This class contains all the information needed to determine what methods
 * should be run. It gets invoked by the TestRunner and then goes through its
 * list of method selectors to decide what methods need to be run.
 * 
...
public class RunInfo implements Serializable {
	...
	transient private List<MethodSelectorDescriptor> m_methodSelectors = Lists
			.newArrayList();

	public void addMethodSelector(IMethodSelector selector, int priority) {
		Utils.log("RunInfo", 3, "Adding method selector: " + selector
				+ " priority: " + priority);
	...
	/**
	 * @return true as soon as we fond a Method Selector that returns true for
	 *         the method "tm".
	 */
	public boolean includeMethod(ITestNGMethod tm, boolean isTestMethod) {
		...
		for (MethodSelectorDescriptor mds : m_methodSelectors) {
			// If we found any negative priority, we break as soon as we
			// encounter
