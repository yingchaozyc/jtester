...
public class MethodInstance implements IMethodInstance {
	...
	/**
	 * ToStringHelper更方便&人性化的输出。
	 */
	@Override
	public String toString() {
		return Objects.toStringHelper(getClass()).add("method", m_method)
				.add("instance", getInstance()).toString();
	}
	...
	public static final Comparator<IMethodInstance> SORT_BY_INDEX = new Comparator<IMethodInstance>() {
		...
		public int compare(IMethodInstance o1, IMethodInstance o2) {
			...
			// If the two methods are not in the same <test>, we can't compare
			// them
			...
			// If the two methods are in the same <class>, compare them by their
			// method
			...
			// This can happen if these classes came from a @Factory, in which
			// case, they
			...
			if (!class1.getName().equals(class2.getName())) {
				...
				XmlInclude include1 = findXmlInclude(
						class1.getIncludedMethods(), o1.getMethod()
								.getMethodName());
				XmlInclude include2 = findXmlInclude(
						class2.getIncludedMethods(), o2.getMethod()
								.getMethodName());
		...
		private XmlInclude findXmlInclude(List<XmlInclude> includedMethods,
				String methodName) {
