...
/**
 * FIXME: reduce contention when this class is used through parallel invocation
 * due to invocationCount and threadPoolSize by not invoking the @BeforeClass
 * and @AfterClass which are already invoked on the original method.
 * 
...
public class TestMethodWorker implements IWorker<ITestNGMethod> {
	...
	public TestMethodWorker(IInvoker invoker, IMethodInstance[] testMethods,
			XmlSuite suite, Map<String, String> parameters,
			ITestNGMethod[] allTestMethods,
			ConfigurationGroupMethods groupMethods,
			ClassMethodMap classMethodMap, ITestContext testContext) {
		m_invoker = invoker;
	...
	public String toString() {
		StringBuilder result = new StringBuilder("[Worker thread:"
				+ Thread.currentThread().getId() + " priority:" + getPriority()
				+ " ");
	...
	protected void invokeTestMethods(ITestNGMethod tm, Object[] instances,
		...
		// more efficient)
		List<ITestResult> testResults = m_invoker.invokeTestMethods(
				tm,
				m_allTestMethods,
	...
	protected void invokeBeforeClassMethods(ITestClass testClass,
		...
		if ((null == m_classMethodMap)
				|| (null == m_classMethodMap.getInvokedBeforeClassMethods())) {
			return;
		}
		...
		Map<ITestClass, Set<Object>> invokedBeforeClassMethods = m_classMethodMap
			...
			for (Object instance : mi.getInstances()) {
				if (!instances.contains(instance)) {
					...
					m_invoker.invokeConfigurations(testClass,
							testClass.getBeforeClassMethods(), m_suite,
							m_parameters, null, /* no parameter values */
							instance);
				}
			}
		}
	...
	protected void invokeAfterClassMethods(ITestClass testClass,
		...
		// used for parallel case when BeforeClass were already invoked
		if ((null == m_classMethodMap)
				|| (null == m_classMethodMap.getInvokedAfterClassMethods())) {
			return;
		}
		...
		if (m_classMethodMap.removeAndCheckIfLast(tm, mi.getInstance())) {
			Map<ITestClass, Set<Object>> invokedAfterClassMethods = m_classMethodMap
			...
			for (Object inst : invokeInstances) {
				m_invoker.invokeConfigurations(testClass, afterClassMethods,
						m_suite, m_parameters, null, /* no parameter values */
						inst);
			}
		}
	}
	...
	/**
	 * The priority of a worker is the priority of the first method it's going
	 * to run.
	 */
	@Override
	public int getPriority() {
		return m_methodInstances.length > 0 ? m_methodInstances[0].getMethod()
				.getPriority() : 0;
	}
}
...
class SingleTestMethodWorker extends TestMethodWorker {
	private static final ConfigurationGroupMethods EMPTY_GROUP_METHODS = new ConfigurationGroupMethods(
			new ITestNGMethod[0], new HashMap<String, List<ITestNGMethod>>(),
			new HashMap<String, List<ITestNGMethod>>());

	public SingleTestMethodWorker(IInvoker invoker, MethodInstance testMethod,
			XmlSuite suite, Map<String, String> parameters,
			ITestNGMethod[] allTestMethods, ITestContext testContext) {
		super(invoker, new MethodInstance[] { testMethod }, suite, parameters,
				allTestMethods, EMPTY_GROUP_METHODS, null, testContext);
	}
}