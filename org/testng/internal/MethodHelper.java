...
public class MethodHelper {
	 ...
	 * @param forTests
	 *            true for test methods, false for configuration methods
	 * @param runInfo
	 * @param finder
	 *            annotation finder
	 * @param unique
	 *            true for unique methods, false otherwise
	...
	public static ITestNGMethod[] collectAndOrderMethods(
		...
		MethodGroupsHelper.collectMethodsByGroup(
				methods.toArray(new ITestNGMethod[methods.size()]), forTests,
				includedMethods, outExcludedMethods, runInfo, finder, unique);

		return sortMethods(forTests, includedMethods, finder).toArray(
				new ITestNGMethod[] {});
	}
	 ...
	 *            TestNG method
	 * @param methods
	 *            list of methods to search for depended upon methods
	...
	protected static ITestNGMethod[] findDependedUponMethods(ITestNGMethod m,
		...
		for (String fullyQualifiedRegexp : m.getMethodsDependedUpon()) {
			...
			if (null != fullyQualifiedRegexp) {
				// Escapes $ in regexps as it is not meant for end - line
				// matching, but inner class matches.
				...
				for (ITestNGMethod method : methods) {
					...
					String methodName = usePackage ? calculateMethodCanonicalName(thisMethod)
							: thisMethodName;
					Pair<String, String> cacheKey = Pair.create(regexp,
							methodName);
	...
	/**
	 * Finds method based on regex and TestNGMethod. If regex doesn't represent
	 * the class name, uses the TestNG method's class name.
	 * 
	 * @param testngMethod
	 *            TestNG method
	 * @param regExp
	 *            regex representing a method and/or related class name
	 * @return
	 */
	private static Method findMethodByName(ITestNGMethod testngMethod,
			String regExp) {
		if (regExp == null) {
			return null;
		}
		...
		if (lastDot == -1) {
			className = testngMethod.getMethod().getDeclaringClass()
					.getCanonicalName();
			methodName = regExp;
		} else {
		...
		try {
			...
			// only logging
			Utils.log("MethodHelper", 3,
					"Caught exception while searching for methods using regex");
		}
		return null;
	}
	 ...
	 * syntax)
	 */
	protected static ExpectedExceptionsHolder findExpectedExceptions(
			IAnnotationFinder finder, Method method) {
		ExpectedExceptionsHolder result = null;
		IExpectedExceptionsAnnotation expectedExceptions = (IExpectedExceptionsAnnotation) finder
				.findAnnotation(method, IExpectedExceptionsAnnotation.class);
		// Old syntax
		if (expectedExceptions != null) {
			result = new ExpectedExceptionsHolder(
					expectedExceptions.getValue(), ".*");
		} else {
			// New syntax
			ITestAnnotation testAnnotation = (ITestAnnotation) finder
					.findAnnotation(method, ITestAnnotation.class);
	...
	protected static boolean isEnabled(Class<?> objectClass,
			IAnnotationFinder finder) {
		ITestAnnotation testClassAnnotation = AnnotationHelper.findTest(finder,
				objectClass);
	...
	protected static boolean isEnabled(Method m, IAnnotationFinder finder) {
		...
		if (null == annotation) {
			annotation = AnnotationHelper.findTest(finder,
					m.getDeclaringClass());
		}
	 ...
	 */
	public static List<ITestNGMethod> uniqueMethodList(
	...
	private static Graph<ITestNGMethod> topologicalSort(
			ITestNGMethod[] methods, List<ITestNGMethod> sequentialList,
		...
		for (ITestNGMethod m : methods) {
			...
			if (methodsDependedUpon.length > 0) {
				ITestNGMethod[] methodsNamed = MethodHelper
						.findDependedUponMethods(m, methods);
			...
			if (groupsDependedUpon.length > 0) {
				for (String group : groupsDependedUpon) {
					ITestNGMethod[] methodsThatBelongToGroup = MethodGroupsHelper
	...
	private static String calculateMethodCanonicalName(Method m) {
		...
		// 这尼玛也不是packageName了吧，是 packageName+methodName
		String packageName = m.getDeclaringClass().getName() + "."
				+ m.getName();
		...
		// 又来了，这尼玛也不是packageName了吧，是 packageName+methodName
	...
	private static List<ITestNGMethod> sortMethods(boolean forTests,
		...
		// Fix the method inheritance if these are @Configuration methods to
		// make
		// sure base classes are invoked before child classes if 'before' and
		// the
	...
	protected static Iterator<Object[]> createArrayIterator(
			final Object[][] objects) {
		ArrayIterator result = new ArrayIterator(objects);
		return result;
	}

	protected static String calculateMethodCanonicalName(Class<?> methodClass,
			String methodName) {
	...
	protected static long calculateTimeOut(ITestNGMethod tm) {
		long result = tm.getTimeOut() > 0 ? tm.getTimeOut() : tm
