...
public class MethodGroupsHelper {
	...
	/**
	 * Collect all the methods that belong to the included groups and exclude
	 * all the methods that belong to an excluded group.
	 */ 
	static void collectMethodsByGroup(ITestNGMethod[] methods,
			boolean forTests, List<ITestNGMethod> outIncludedMethods,
			List<ITestNGMethod> outExcludedMethods, RunInfo runInfo,
		...
		for (ITestNGMethod tm : methods) {
			...
			//
			if (forTests) {
				in = MethodGroupsHelper.includeMethod(
			...
			//
				...
				IConfigurationAnnotation annotation = AnnotationHelper
						.findConfiguration(finder, m);
				if (annotation.getAlwaysRun()) {
					if (!unique
							|| !MethodGroupsHelper.isMethodAlreadyPresent(
									outIncludedMethods, tm)) {
						in = true;
					}
				} else {
					in = MethodGroupsHelper.includeMethod(
							AnnotationHelper.findTest(finder, m), runInfo, tm,
							forTests, unique, outIncludedMethods);
				}
			}
			if (in) {
	...
	private static boolean includeMethod(ITestOrConfiguration annotation,
			RunInfo runInfo, ITestNGMethod tm, boolean forTests,
			boolean unique, List<ITestNGMethod> outIncludedMethods) {
		boolean result = false;

		if (MethodHelper.isEnabled(annotation)) {
			if (runInfo.includeMethod(tm, forTests)) {
				if (unique) {
					if (!MethodGroupsHelper.isMethodAlreadyPresent(
							outIncludedMethods, tm)) {
						result = true;
					}
				} else {
	...
	public static Map<String, List<ITestNGMethod>> findGroupsMethods(
		...
		for (ITestClass cls : classes) {
			ITestNGMethod[] methods = before ? cls.getBeforeGroupsMethods()
					: cls.getAfterGroupsMethods();
			for (ITestNGMethod method : methods) {
				for (String group : before ? method.getBeforeGroups() : method
						.getAfterGroups()) {
					...
					// BeforeGroups/AfterGroups methods should only be invoked
					// once.
					// I should probably use a map instead of a list for a
					// contains(), but
	...
	protected static void findGroupTransitiveClosure(
		...
		for (ITestNGMethod m : includedMethods) {
			runningMethods.put(m, m);   // 运行时候的方法？ TODO 扔进去了
		}
			...
			for (ITestNGMethod m : includedMethods) {
				...
				for (String g : ig) {
					if (!runningGroups.containsKey(g)) {
						// Found a new included group, add all the methods it
						// contains to
						// our outMethod closure
						runningGroups.put(g, g);
						ITestNGMethod[] im = MethodGroupsHelper
								.findMethodsThatBelongToGroup(m, allMethods.toArray(new ITestNGMethod[allMethods.size()]), g);
	...
	private static ITestNGMethod findMethodNamed(String tm,
			List<ITestNGMethod> allMethods) {
	 ...
	 * 
	 * @param method
	 *            if no group is found, group regex is set as this method's
	 *            missing group
	 * @param methods
	 *            list of methods to search
	 * @param groupRegexp
	 *            regex representing the group
	 * 
	 * @return all the methods that belong to the group specified by the regular
	 *         expression groupRegExp. methods[] is the list of all the methods
	 *         we are choosing from and method is the method that owns the
	 *         dependsOnGroups statement (only used if a group is missing to
	 *         flag an error on that method).
	 */
