...
public class XmlTest implements Serializable, Cloneable {
	...
	private Map<String, List<Integer>> m_failedInvocationNumbers = null; // lazily
																			// initialized
	...
	/**
	 * @return the includedGroups. Note: do not modify the returned value, use
	 *         {@link #addIncludedGroup(String)}.
	 */
	...
	public String toXml(String indent) {
		...
		if (m_isJUnit != null) {
			XmlUtils.setProperty(p, "junit", m_isJUnit.toString(),
					XmlSuite.DEFAULT_JUNIT.toString());
		}
		if (m_parallel != null) {
			XmlUtils.setProperty(p, "parallel", m_parallel,
					XmlSuite.DEFAULT_PARALLEL);
		}
		if (m_verbose != null) {
			XmlUtils.setProperty(p, "verbose", m_verbose.toString(),
					XmlSuite.DEFAULT_VERBOSE.toString());
		}
		if (null != m_timeOut) {
		...
		if (!m_metaGroups.isEmpty() || !m_includedGroups.isEmpty()
			...
			// group dependencies
			if (m_xmlDependencyGroups != null
					&& !m_xmlDependencyGroups.isEmpty()) {
				xsb.push("dependencies");
				for (Map.Entry<String, String> entry : m_xmlDependencyGroups
						.entrySet()) {
					xsb.addEmptyElement("group", "name", entry.getKey(),
							"depends-on", entry.getValue());
				}
	...
	/**
	 * Clone the <TT>source</TT> <CODE>XmlTest</CODE> by including: - test
	 * attributes - groups definitions - parameters
	 * 
	...
	public List<Integer> getInvocationNumbers(String method) {
			...
			for (XmlClass c : getXmlClasses()) {
				for (XmlInclude xi : c.getIncludedMethods()) {
					...
					if (invocationNumbers.size() > 0) {
						...
						m_failedInvocationNumbers.put(methodName,
								invocationNumbers);
					}
				}
			}
		...
		if (result == null) {
			// Don't use emptyList here since this list might end up receiving
			// values if
			// the test run fails.
	 ...
	 * internally and represents the order in which this test tag was found in
	 * its &lt;suite&gt; tag. It's used to calculate the ordering of the tests
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((m_failedInvocationNumbers == null) ? 0
						: m_failedInvocationNumbers.hashCode());
		result = prime
				* result
	...
	public boolean equals(Object obj) {
		...
		if (m_isJUnit == null) {
			if (other.m_isJUnit != null
					&& !other.m_isJUnit.equals(XmlSuite.DEFAULT_JUNIT))
				return XmlSuite.f();
