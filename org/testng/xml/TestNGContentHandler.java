...
public class TestNGContentHandler extends DefaultHandler {
		...
		if (start) {
			...
			if (parallel != null) {
				if (isValidParallel(parallel)) {
					...
					Utils.log("Parser", 1,
							"[WARN] Unknown value of attribute 'parallel' at suite level: '"
									+ parallel + "'.");
				}
			}
			String configFailurePolicy = attributes
					.getValue("configfailurepolicy");
			if (null != configFailurePolicy) {
				if (XmlSuite.SKIP.equals(configFailurePolicy)
						|| XmlSuite.CONTINUE.equals(configFailurePolicy)) {
			...
			if (groupByInstances != null) {
				m_currentSuite.setGroupByInstances(Boolean.valueOf(
						groupByInstances).booleanValue());
			}
			String skip = attributes.getValue("skipfailedinvocationcounts");
			if (skip != null) {
				m_currentSuite.setSkipFailedInvocationCounts(Boolean
						.valueOf(skip));
			}
			...
			String dataProviderThreadCount = attributes
					.getValue("data-provider-thread-count");
			if (null != dataProviderThreadCount) {
				m_currentSuite.setDataProviderThreadCount(Integer
						.parseInt(dataProviderThreadCount));
			}
			...
			if (null != objectFactory && m_loadClasses) {
				try {
					m_currentSuite.setObjectFactory((ITestObjectFactory) Class
							.forName(objectFactory).newInstance());
				} catch (Exception e) {
					Utils.log("Parser", 1,
							"[ERROR] Unable to create custom object factory '"
									+ objectFactory + "' :" + e);
				}
			}
			...
			String allowReturnValues = attributes
					.getValue("allow-return-values");
			if (allowReturnValues != null) {
				m_currentSuite.setAllowReturnValues(Boolean
						.valueOf(allowReturnValues));
			}
		} else {
	...
	private void xmlDefine(boolean start, Attributes attributes) {
		if (start) {
			...
			m_currentTest.addMetaGroup(m_currentMetaGroupName,
					m_currentMetaGroup);
	...
	private void xmlTest(boolean start, Attributes attributes) {
		if (start) {
			...
			String skip = attributes.getValue("skipfailedinvocationcounts");
			if (skip != null) {
				m_currentTest.setSkipFailedInvocationCounts(Boolean.valueOf(
						skip).booleanValue());
			}
			String groupByInstances = attributes.getValue("group-by-instances");
			if (groupByInstances != null) {
				m_currentTest.setGroupByInstances(Boolean.valueOf(
						groupByInstances).booleanValue());
			}
			String preserveOrder = attributes.getValue("preserve-order");
			if (preserveOrder != null) {
			...
			if (null != m_currentTestParameters
					&& m_currentTestParameters.size() > 0) {
	...
	public void startElement(String uri, String localName, String qName,
		...
		// ppp("START ELEMENT uri:" + uri + " sName:" + localName + " qName:" +
		// qName +
		// " " + attributes);
		if ("suite".equals(qName)) {
			...
			if (null != m_currentClasses) {
				m_currentClass = new XmlClass(name, m_currentClassIndex++,
						m_loadClasses);
		...
		if (start) {
			...
			if (null != m_currentIncludedMethods) {
				...
				if (!Utils.isStringEmpty(in)) {
					include = new XmlInclude(name, stringToList(in),
							m_currentIncludeIndex++);
				} else {
					include = new XmlInclude(name, m_currentIncludeIndex++);
				}
				for (Map.Entry<String, String> entry : m_currentInclude.parameters
						.entrySet()) {
	...
	private static String expandValue(String value) {
		...
		while ((startIndex = value.indexOf("${", startPosition)) > -1
			...
			if (result == null) {
				result = new StringBuffer(value.substring(startPosition,
						startIndex));
			} else {
