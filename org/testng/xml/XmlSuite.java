...
public class XmlSuite implements Serializable, Cloneable {
	...
	/**
	 * By default, a method failing will cause all instances of that class to
	 * skip
	 */
	 ...
	 * should be invoked any time there is a change in the state of this suite
	 * that would affect the parameter list.<br>
	 * NOTE: Currently being invoked after a parent suite is added or if
	 * parameters for this suite are updated.
	 */
	private void updateParameters() {
		/*
		 * Whatever parameters are set by user or via XML, should be updated
		 * using parameters from parent suite, if it exists. Parameters from
		 * this suite override the same named parameters from parent suite.
		 */
	 ...
	 * suite. Also, parameters from this suite override the same named
	 * parameters from parent suite.
	 */
	...
	public String toXml() {
		...
		if (getVerbose() != null) {
			XmlUtils.setProperty(p, "verbose", getVerbose().toString(),
					DEFAULT_VERBOSE.toString());
		}
		...
		XmlUtils.setProperty(p, "configfailurepolicy",
				getConfigFailurePolicy(), DEFAULT_CONFIG_FAILURE_POLICY);
		XmlUtils.setProperty(p, "thread-count",
				String.valueOf(getThreadCount()),
				DEFAULT_THREAD_COUNT.toString());
		XmlUtils.setProperty(p, "data-provider-thread-count",
				String.valueOf(getDataProviderThreadCount()),
				DEFAULT_DATA_PROVIDER_THREAD_COUNT.toString());
		if (!DEFAULT_JUNIT.equals(m_isJUnit)) {
			p.setProperty("junit", m_isJUnit != null ? m_isJUnit.toString()
					: "false"); // TESTNG-141
		}
		XmlUtils.setProperty(p, "skipfailedinvocationcounts",
				m_skipFailedInvocationCounts.toString(),
				DEFAULT_SKIP_FAILED_INVOCATION_COUNTS.toString());
		if (null != m_objectFactory) {
			p.setProperty("object-factory", m_objectFactory.getClass()
					.getName());
		}
		XmlUtils.setProperty(p, "allow-return-values",
				String.valueOf(getAllowReturnValues()),
	...
	public String toString() {
		StringBuffer result = new StringBuffer("[Suite: \"").append(m_name)
				.append("\" ");
	...
	/**
	 * {@inheritDoc} Note that this is not a full clone: XmlTest children are
	 * not cloned by this method.
	 */
	@Override
	public Object clone() {
	...
	/**
	 * Returns the timeout as a long value specifying the default value to be
	 * used if no timeout was specified.
	 * 
	 * @param def
	 *            the the default value to be used if no timeout was specified.
	 * @return the timeout as a long value specifying the default value to be
	 *         used if no timeout was specified.
	 */
	...
	public int getDataProviderThreadCount() {
		...
		if (s != null) {
			try {
				...
				System.err
						.println("Parsing System property 'dataproviderthreadcount': "
								+ nfe);
			}
		}
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((m_methodSelectors == null) ? 0 : m_methodSelectors
						.hashCode());
	 ...
	 * Such a value is considered equal to another one if that other one is
	 * null.
	 */
	...
	/**
	 * @return Returns the includedGroups. Note: do not modify the returned
	 *         value, use {@link #addIncludedGroup(String)}.
	 */
	...
	/**
	 * @return Returns the excludedGroups. Note: do not modify the returned
	 *         value, use {@link #addExcludedGroup(String)}.
	 */
	...
	// @OnElementList(tag = "method-selectors", attributes = { "language",
	// "name", "priority" })
	public void onMethodSelectorElement(String language, String name,
			String priority) {
