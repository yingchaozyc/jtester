...
public class SuiteHTMLReporter implements IReporter {
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
		...
		for (int i = 0; i < suites.size(); i++) {
			...
			generateMethodsChronologically(xmlSuite, suite,
					METHODS_CHRONOLOGICAL, false);
			generateMethodsChronologically(xmlSuite, suite,
					METHODS_ALPHABETICAL, true);
	...
	private void generateXmlFile(XmlSuite xmlSuite, ISuite suite) {
		String content = xmlSuite.toXml().replaceAll("<", "&lt;")
		...
		sb.append("<head><title>").append("testng.xml for ")
				...
				.append(content).append("</tt></body></html>");
	...
	private void generateIndex(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			String cls = failedTests > 0 ? "invocation-failed"
					: (passedTests > 0 ? "invocation-passed"
							: "invocation-failed");
			suiteBuf.append("<tr align='center' class='").append(cls)
					.append("'>").append("<td><a href='").append(name)
			...
			suiteBuf.append("<td>" + passedTests + "</td>")
					...
					.append("<td><a href='").append(name).append("/")
					.append(TESTNG_XML).append("'>Link").append("</a></td>")
					.append("</tr>");

		}

		String cls = totalFailedTests > 0 ? "invocation-failed"
				: (totalPassedTests > 0 ? "invocation-passed"
						: "invocation-failed");
		sb.append("<tr align='center' class='").append(cls).append("'>")
				.append("<td><em>Total</em></td>").append("<td><em>")
				.append(totalPassedTests).append("</em></td>")
				.append("<td><em>").append(totalFailedTests)
				.append("</em></td>").append("<td><em>")
				.append(totalSkippedTests).append("</em></td>")
	...
	private void generateExcludedMethodsReport(XmlSuite xmlSuite, ISuite suite) {
		...
		for (ITestNGMethod method : excluded) {
			...
			if (m != null) {
				sb2.append("<tr><td>").append(
						m.getDeclaringClass().getName() + "." + m.getName());
				String description = method.getDescription();
				if (isStringNotEmpty(description)) {
					sb2.append("<br/>").append(SP2).append("<i>")
							.append(description).append("</i>");
				}
				sb2.append("</td></tr>\n");
			}
		}
		...
		Utils.writeFile(getOutputDirectory(xmlSuite), METHODS_NOT_RUN,
				sb2.toString());
	}
	...
	private void generateReporterOutput(XmlSuite xmlSuite, ISuite suite) {
		...
		Utils.writeFile(getOutputDirectory(xmlSuite), REPORTER_OUTPUT,
				sb.toString());
	}
	...
	private String generateClass(ITestClass cls) {
		...
		sb.append("<tr>\n").append("<td>").append(cls.getRealClass().getName())
				.append("</td>\n").append("<td>&nbsp;</td>")
				.append("<td>&nbsp;</td>").append("</tr>\n");

		String[] tags = new String[] { "@Test", "@BeforeClass",
				"@BeforeMethod", "@AfterMethod", "@AfterClass" };
		ITestNGMethod[][] methods = new ITestNGMethod[][] {
				cls.getTestMethods(), cls.getBeforeClassMethods(),
				cls.getBeforeTestMethods(), cls.getAfterTestMethods(),
				cls.getAfterClassMethods() };

		for (int i = 0; i < tags.length; i++) {
			sb.append("<tr>\n").append("<td align='center' colspan='3'>")
					.append(tags[i]).append("</td>\n").append("</tr>\n")
	...
	private String dumpMethods(ITestNGMethod[] testMethods) {
		...
		for (ITestNGMethod m : testMethods) {
			sb.append("<tr>\n");
			sb.append("<td>&nbsp;</td>\n").append("<td>")
					.append(m.getMethodName()).append("</td>\n");
	...
	private void generateMethodsChronologically(XmlSuite xmlSuite,
			ISuite suite, String outputFileName, boolean alphabetical) {
		StringBuffer sb = new StringBuffer();

		sb.append("<h2>Methods run, sorted chronologically</h2>");
		sb.append("<h3>" + BEFORE + " means before, " + AFTER
				+ " means after</h3><p/>");
		...
		Collection<IInvokedMethod> invokedMethods = suite
		...
		if (alphabetical) {
			...
			Comparator<? super ITestNGMethod> alphabeticalComparator = new Comparator() {
				...
				public int compare(Object o1, Object o2) {
					...
					return m1.getTestMethod().getMethodName()
							.compareTo(m2.getTestMethod().getMethodName());
				}
			};
		...
		for (IInvokedMethod iim : invokedMethods) {
			...
			if (!addedHeader) {
				table.append("<table border=\"1\">\n").append("<tr>")
						.append("<th>Time</th>").append("<th>Delta (ms)</th>")
						...
						.append("<th>Thread</th>").append("<th>Instances</th>")
						.append("</tr>\n");
				addedHeader = true;
			}
			...
			String configurationClassMethod = isClassConfiguration ? (bc ? BEFORE
					: AFTER)
					+ methodName
					: SP;
			String configurationTestMethod = isTestConfiguration ? (bt ? BEFORE
					: AFTER) + methodName : SP;
			String configurationGroupsMethod = isGroupsConfiguration ? (bg ? BEFORE
					: AFTER)
					+ methodName
					: SP;
			String configurationSuiteMethod = isSuiteConfiguration ? (bs ? BEFORE
					: AFTER)
					+ methodName
					: SP;
			String setUpOrTearDownMethod = isSetupOrTearDown ? (setUp ? BEFORE
					: AFTER) + methodName : SP;
			...
			table.append("<tr bgcolor=\"" + createColor(tm) + "\">")
					...
					.append("  <td>").append(iim.getDate() - startDate)
					.append("</td> ").append(td(configurationSuiteMethod))
					.append(td(configurationTestMethod))
					.append(td(configurationClassMethod))
					.append(td(configurationGroupsMethod))
					.append(td(setUpOrTearDownMethod)).append(td(testMethod))
					.append("  <td>").append(tm.getId()).append("</td> ")
					.append("  <td>").append(instances).append("</td> ")
					.append("</tr>\n");
			Utils.appendToFile(getOutputDirectory(xmlSuite), outputFileName,
					table.toString());
		}
		Utils.appendToFile(getOutputDirectory(xmlSuite), outputFileName,
				"</table>\n");
	...
	private String createColor(ITestNGMethod tm) {
		...
		long color = tm.getRealClass() != null ? tm.getRealClass().hashCode() & 0xffffff
				: 0xffffff;
		long[] rgb = { ((color & 0xff0000) >> 16) & 0xff,
				((color & 0x00ff00) >> 8) & 0xff, color & 0xff };
		// Not too dark
	...
	private void generateMethodsAndGroups(XmlSuite xmlSuite, ISuite suite) {
		...
		if (groups.size() > 0) {
			...
			for (String group : groupNames) {
				...
				Map<ITestNGMethod, ITestNGMethod> uniqueMethods = Maps
						.newHashMap();
	...
	private void generateIndex(XmlSuite xmlSuite, ISuite sr) {
		StringBuffer index = new StringBuffer()
				.append("<html><head><title>Results for " + sr.getName()
						+ "</title></head>\n")
		...
		Utils.writeFile(getOutputDirectory(xmlSuite), "index.html",
				index.toString());
	}
	...
	private void generateMain(XmlSuite xmlSuite, ISuite sr) {
		...
		Utils.writeFile(getOutputDirectory(xmlSuite), "main.html",
				index.toString());
	}
   ...
   */
	private void generateTableOfContents(XmlSuite xmlSuite, ISuite suite) {
		...
		tableOfContents
				.append("<body>\n")
				.append("<h3><p align=\"center\">" + makeTitle(suite)
						+ "</p></h3>\n")
				...
				.append(m_classes.size() + " "
						+ pluralize(m_classes.size(), "class"))
				.append("</a>")
				.append("</td>\n")
				.append("<td>" + methodCount + " "
						+ pluralize(methodCount, "method") + ":<br/>\n")
				.append("&nbsp;&nbsp;<a target='mainFrame' href='")
				...
				.append("'>not run (" + suite.getExcludedMethods().size()
						+ ")</a>")
				.append("</td>\n")
				.append("</tr>\n")
				...
				.append(GROUPS)
				.append("'>")
				.append(groupCount + pluralize(groupCount, " group")
						+ "</a></td>\n")
				.append("<td><a target='mainFrame' href='")
				.append(REPORTER_OUTPUT).append("'>reporter output</a></td>\n")
		...
		Utils.writeFile(getOutputDirectory(xmlSuite), "toc.html",
				tableOfContents.toString());
	}

	private String pluralize(int count, String singular) {
		return count > 1 ? (singular.endsWith("s") ? singular + "es" : singular
				+ "s") : singular;
	}
	...
	private void generateSuiteResult(String suiteName, ISuiteResult sr,
			String cssClass, StringBuffer tableOfContents,
			String outputDirectory) {
		ITestContext tc = sr.getTestContext();
		int passed = tc.getPassedTests().size();
		int failed = tc.getFailedTests().size();
		...
		tableOfContents
				...
				.append("  <a href='" + baseFile
						+ ".html' target='mainFrame'>Results</a>\n")
				// .append("  <a href=\"" + baseFile +
				// ".out\" target=\"mainFrame\"\">Output</a>\n")
				// .append("&nbsp;&nbsp;<a href=\"file://" + baseFile +
				// ".properties\" target=\"mainFrame\"\">Property file</a><br>\n")
	...
	private void generateSuites(XmlSuite xmlSuite, ISuite suite) {
		...
		for (ISuiteResult sr : suiteResults.values()) {
			...
			Utils.writeFile(getOutputDirectory(xmlSuite), testContext.getName()
