...
public class Main implements IReporter {
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
		...
		List<INavigatorPanel> panels = Arrays.<INavigatorPanel> asList(
				new TestNgXmlPanel(m_model), new TestPanel(m_model),
				new GroupPanel(m_model), new TimesPanel(m_model),
				new ReporterPanel(m_model), new IgnoredMethodsPanel(m_model),
			...
			if (header == null) {
				...
				for (String fileName : RESOURCES) {
					InputStream is = getClass().getResourceAsStream(
							"/" + fileName);
					if (is == null) {
						throw new AssertionError("Couldn't find resource: "
								+ fileName);
					}
